...
| [`linkstack/`](linkstack/) | LinkStack | Aktiv | Eigenständiger ChatGPT-/MCP-Connector für die vollständige Bratonien-LinkStack-REST-API mit serverseitig durch LinkStack erzwungenen Fine-Grained Permissions. |
| [`affine/`](affine/) | AFFiNE | In Aufbau | Geplanter Connector auf Basis des nativen AFFiNE-MCP mit Workspace-Credentials, `READ_ONLY`/`READ_WRITE` und zentraler Bratonien-MCP-Integration. |
